...
from data_models.proactive_request import ProactiveRequest
...
def create_hero_card_with_image(news: ProactiveRequest) -> Attachment:
    card = HeroCard(
        title=news.name.replace('#', '\\#').replace('(', '\\(').replace('>','\\>').replace(')', '\\)').replace('-', '\\-').replace('.', '\\.').replace('"',
                                                                                                                '\\"'),
        subtitle=news.date.replace('#', '\\#').replace('(', '\\(').replace(')', '\\)').replace('>','\\>').replace('-', '\\-').replace('.', '\\.').replace('"',
                                                                                                                   '\\"'),
        text=news.description.replace('#', '\\#').replace('(', '\\(').replace(')', '\\)').replace('>','\\>').replace('-', '\\-').replace('.', '\\.').replace(
            '"', '\\"'),
        images=[
            CardImage(
                url=news.image
